...
function preload(){
  soundFile = loadSound("assets/CowMoo.mp3"); //mp3, wave, webaudio
  vid = createVideo("assets/cow.mp4"); // creates an HTML video tag, canvas will shift.
}
...
function setup() {
  ...
  // soundFile.play(); // mousepress is only once
  vid.hide();
...
// this is if youy have diff buttons, frward, pause, stop, play, etc.
function mousePressed(){
 //  for(var i = 0; i < buttonArray.length; i++){
 //    if(buttonArray[i].check(mouseX, mouseY)){
 // if (i == 0 || buttonArray[i].name == ){ // constantly checking
 //   //over the green btn
 // }else if (i==1){
 //   //over the red btn
 // }
